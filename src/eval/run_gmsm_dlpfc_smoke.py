"""GMSM smoke test: DLPFC slice 151673, 1 seed.

Never run before this pass. Saves output to outputs/logs/gmsm_dlpfc_smoke.json.
"""
...
OUTPUT_PATH = Path(__file__).resolve().parents[2] / "outputs" / "logs" / "gmsm_dlpfc_smoke.json"
SMOKE_SLICE = "151673"
SMOKE_SEED = 0
...
def main():
    ...
    print(f"\n=== GMSM Smoke Test: slice {SMOKE_SLICE}, seed {SMOKE_SEED} ===")
    print(f"  n_spots={adata.n_obs}, n_layers={n_layers}")

    model, trained_adata, history = train_gmsm_model(
        adata.copy(), seed=SMOKE_SEED, device=device, epochs=600, log_every=100, verbose=True,
    )

    embedding = trained_adata.obsm["X_gmsm"]
    ...
    print(f"  Collapsed:        {collapsed}  (local_slots={final['local_slots_used']}, global_slots={final['global_slots_used']})")
    print(f"  Mean gate:        {final['mean_gate']:.3f}")
