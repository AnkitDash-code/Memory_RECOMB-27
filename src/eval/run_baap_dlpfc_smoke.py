"""BAAP smoke test: DLPFC slice 151673, 1 seed.

Verifies:
  1. Loss decreases over 600 epochs.
  2. Usage entropy stays high (no codebook collapse).
  3. Effective hop depth and hop weight distribution are logged.
  4. ARI vs ground-truth spatial layers on slice 151673.

Saves output to outputs/logs/baap_dlpfc_smoke.json.
"""
...
OUTPUT_PATH = Path(__file__).resolve().parents[2] / "outputs" / "logs" / "baap_dlpfc_smoke.json"
SMOKE_SLICE = "151673"
SMOKE_SEED = 0
...
def main():
    ...
    print(f"\n=== BAAP Smoke Test: slice {SMOKE_SLICE}, seed {SMOKE_SEED} ===")
    print(f"  n_spots={adata.n_obs}, n_layers={n_layers}")

    model, trained_adata, history = train_baap_model(
        adata.copy(),
        seed=SMOKE_SEED,
        device=device,
        epochs=600,
    ...
    embedding = trained_adata.obsm["X_baap"]
