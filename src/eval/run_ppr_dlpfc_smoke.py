"""PPR smoke test: DLPFC slice 151673, 1 seed, default alpha=0.2.

An earlier informal session reported ARI~0.598 here (single seed, sanity
check only, never persisted). This script reproduces that check for real
and saves the log.

Saves output to outputs/logs/ppr_dlpfc_smoke.json.
"""
...
OUTPUT_PATH = Path(__file__).resolve().parents[2] / "outputs" / "logs" / "ppr_dlpfc_smoke.json"
SMOKE_SLICE = "151673"
SMOKE_SEED = 0
ALPHA = 0.2
...
def main():
    ...
    print(f"\n=== PPR Smoke Test: slice {SMOKE_SLICE}, seed {SMOKE_SEED}, alpha={ALPHA} ===")
    print(f"  n_spots={adata.n_obs}, n_layers={n_layers}")

    model, trained_adata, history = train_ppr_model(
        adata.copy(), alpha=ALPHA, seed=SMOKE_SEED, device=device, epochs=600, log_every=100, verbose=True,
    )

    embedding = trained_adata.obsm["X_ppr"]
