"""Training loop for SimVQMemoryAutoencoder (Stage 2). Mirrors
train_spatial_address_model's loop exactly (same loss terms: reconstruction
MSE - lambda_usage * usage_entropy, same logging convention) so any
difference is attributable to the key reparameterization alone.
"""
...
from src.models.simvq_layer import SimVQMemoryAutoencoder
...
def train_simvq_model(
    ...
    for epoch in range(epochs):
        ...
        if epoch % log_every == 0 or epoch == epochs - 1:
            with torch.no_grad():
                keys = model.memory.memory_keys
                key_sim = key_cosine_similarity(keys).item()
    ...
    adata.obsm["X_simvq"] = embedding.cpu().numpy()
